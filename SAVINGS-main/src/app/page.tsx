    ...
    <SidebarProvider>
      <Sidebar className="border-r border-border/60 bg-card/70 backdrop-blur-xl">
        ...
        <SidebarContent className="px-3 py-5">
          ...
          <SidebarMenu>
            <SidebarMenuItem>
              <SidebarMenuButton isActive className="h-11 rounded-xl font-medium"><LayoutDashboard />Dashboard</SidebarMenuButton>
            </SidebarMenuItem>
            <SidebarMenuItem>
              <SidebarMenuButton className="h-11 rounded-xl font-medium" onClick={() => document.getElementById("jusu-rush")?.scrollIntoView({ behavior: "smooth" })}>
                <Gamepad2 />JUSU RUSH
              </SidebarMenuButton>
            </SidebarMenuItem>
          </SidebarMenu>
        ...
        <div className="min-h-screen bg-background text-foreground">
          <header className="sticky top-0 z-30 border-b border-border/50 bg-background/80 backdrop-blur-xl">
            <div className="mx-auto flex h-[72px] max-w-[1600px] items-center justify-between px-4 sm:px-6 lg:px-8">
              <div className="flex items-center gap-3">
                ...
                <div><p className="hidden text-xs font-medium text-muted-foreground sm:block">Your money, organized</p><h1 className="text-xl font-bold tracking-tight sm:text-2xl">Dashboard</h1></div>
              </div>
              <DropdownMenu>
                <DropdownMenuTrigger asChild><Button variant="ghost" className="h-10 w-10 rounded-full p-0"><Avatar className="h-10 w-10 border border-border/60"><AvatarImage src={user.photoURL ?? ""} alt={user.email ?? "Account"} /><AvatarFallback className="bg-primary/10 font-semibold text-primary">{user.isAnonymous ? <UserIcon className="h-5 w-5" /> : getInitials(user.email)}</AvatarFallback></Avatar></Button></DropdownMenuTrigger>
                <DropdownMenuContent className="w-60 rounded-2xl p-2" align="end">
                  ...
                  <DropdownMenuSeparator /><DropdownMenuItem onClick={handleLogout} className="rounded-xl"><LogOut className="mr-2 h-4 w-4" />Log out</DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
            </div>
          </header>
          ...
          <main className="mx-auto max-w-[1600px] px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
            ...
            <section className="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
              <div className="rounded-2xl border border-border/60 bg-card p-5 shadow-sm"><div className="mb-4 flex items-center justify-between"><span className="text-sm font-medium text-muted-foreground">Contributions</span><div className="rounded-xl bg-primary/10 p-2 text-primary"><Wallet className="h-4 w-4" /></div></div><p className="text-2xl font-bold tracking-tight">{savingsLoaded ? contributionCount : "—"}</p><p className="mt-1 text-xs text-muted-foreground">Total deposits recorded</p></div>
              <div className="rounded-2xl border border-border/60 bg-card p-5 shadow-sm"><div className="mb-4 flex items-center justify-between"><span className="text-sm font-medium text-muted-foreground">Average deposit</span><div className="rounded-xl bg-emerald-500/10 p-2 text-emerald-600"><TrendingUp className="h-4 w-4" /></div></div><p className="text-2xl font-bold tracking-tight">{savingsLoaded ? `$${averageContribution.toFixed(2)}` : "—"}</p><p className="mt-1 text-xs text-muted-foreground">Across all recorded contributions</p></div>
              <div className="rounded-2xl border border-border/60 bg-card p-5 shadow-sm"><div className="mb-4 flex items-center justify-between"><span className="text-sm font-medium text-muted-foreground">Latest activity</span><div className="rounded-xl bg-amber-500/10 p-2 text-amber-600"><PiggyBank className="h-4 w-4" /></div></div><p className="truncate text-2xl font-bold tracking-tight">{savingsLoaded && latestContribution ? `$${latestContribution.usdAmount.toFixed(2)}` : savingsLoaded ? "$0.00" : "—"}</p><p className="mt-1 text-xs text-muted-foreground">Most recent contribution</p></div>
            </section>
            ...
            <div className="grid grid-cols-1 gap-6 xl:grid-cols-[320px_minmax(0,1fr)]">
              ...
              <div className="min-w-0 space-y-6"><div className="grid grid-cols-1 gap-6 lg:grid-cols-5"><div className="lg:col-span-3"><SavingsPieChart savings={savings} isLoaded={savingsLoaded} /></div><div className="lg:col-span-2"><SavingsSummaryCard savings={savings} isLoaded={savingsLoaded} /></div></div><SavingsChart savings={savings} isLoaded={savingsLoaded} /><SavingsList savings={savings} isLoaded={savingsLoaded} deleteSaving={deleteSaving} /></div>
            </div>
          </main>
