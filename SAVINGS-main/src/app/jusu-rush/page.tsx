...
export default function JusuRushPage(){
 ...
 useEffect(()=>{
  ...
  const touchMove=(e:TouchEvent)=>{const t=e.touches[0];if(!t)return;const dx=t.clientX-touchStartX;if(Math.abs(dx)>12){game.current.target=Math.max(-1,Math.min(1,game.current.target+(dx>0?1:-1)*.08));touchStartX=t.clientX}e.preventDefault()};
  ...
  const loop=(now:number)=>{
   ...
   if(started.current&&!dead&&!finished){
    ...
    for(const it of track){if(it.hit)continue;const dz=it.z-s.z;if(dz<0||dz>38||Math.abs(it.lane-s.lane)>.38)continue;it.hit=true;if(it.kind==='coin')setScore(v=>v+25);if(it.kind==='boost')s.boost=2.5;if(it.kind==='spring'){s.spring=1;s.speed+=11}if(it.kind==='slow'){s.slow=2;s.speed*=.45}if(it.kind==='kill'){s.speed=0;setDead(true);setStatus('CRASHED')}if(it.kind==='ramp'){s.spring=.8;s.speed+=8}if(it.kind==='hammer')s.speed*=.25}
    if(s.z>=LENGTH){setFinished(true);setStatus('FINISH — YOU WIN')}
   ...
   if(dead||finished){g.fillStyle='rgba(0,0,0,.72)';g.fillRect(0,0,w,h);g.textAlign='center';g.fillStyle=dead?'#ef4444':'#facc15';g.font='900 52px system-ui';g.fillText(dead?'CRASH!':'🏁 YOU WIN',w/2,h*.46);g.fillStyle='#fff';g.font='700 20px system-ui';g.fillText(dead?'Tap REMATCH':'First to 5000m',w/2,h*.54);g.textAlign='left'}
